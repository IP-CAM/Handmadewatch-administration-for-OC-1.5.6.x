	...
	private $error = array(); 
  	...
  	protected function getList() {				
		...
		foreach ($results as $result) {
			...
			if ($result['sku'] == "") {
			$remeshok = $this->model_catalog_product->getRemeshok($result['product_id']);
			$tsiferblat = $this->model_catalog_product->getTsiferblat($result['product_id']);
			$result['sku'] = $remeshok['sku'] . " / " . $tsiferblat['sku'];
			if ($remeshok['quantity'] > $tsiferblat['quantity']) {
				$result['quantity'] = $tsiferblat['quantity'];
			} else {
				$result['quantity'] = $remeshok['quantity'];
			}
		}
      		$this->data['products'][] = array(
