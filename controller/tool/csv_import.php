...
class ControllerToolCsvImport extends Controller {
	...
	public function index() {
		...
		$headings_info = array(
			...
			'csv_import_field_cat1' => 'category1',
			'csv_import_field_cat2' => 'category2',
			'csv_import_field_cat3' => 'category3',
			'csv_import_field_cat4' => 'category4',
			...
			'csv_import_field_status' => 'status',
			'csv_import_field_minqnty' => 'minimum',
			'csv_import_field_vklike' => 'vklike',
			'csv_import_field_vkshare' => 'vkshare',
			'csv_import_field_facebook' => 'facebook',
			'csv_import_field_google' => 'google',
			'csv_import_field_mymir' => 'mymir',
			'csv_import_field_ok' => 'ok',
			'csv_import_field_twitter' => 'twitter',
			'csv_import_field_deliver' => 'deliver',
			'csv_import_field_video' => 'video'
		);
		...
		foreach ($results as $result) {						
			...
			$headings_info['csv_import_field_attribute'.$result['attribute_id']] = 'product_attribute'.$result['attribute_id'];			
		}
		
		//echo $this->request->server['REQUEST_METHOD'];
		//echo isset($this->request->post['csv_import_field_attribute2']);
		foreach ($headings_info as $key => $value) {
		//echo $key . '<br>';
			if (isset($this->request->post[$key])) {
			//echo $key . ' ' . $this->request->post[$key] . '<br>';
...
//echo $this->getNextProduct($file)['name'];
	    while (($raw_prod = $this->getNextProduct($file)) !== FALSE) 
	    {
			...
			//echo $raw_prod[$this->field_names['foto2']];
			...
			foreach (array('price', 'cost') as $price) {
				...
				//echo $raw_prod[$this->field_names[$price]];
			}
			...
			$sd_params['social_discount_vk_like_enabled']  = isset($product['social_discount_vk_like_enabled'])  ? $product['social_discount_vk_like_enabled'] == 'on'  : $sd_params['social_discount_vk_like_enabled'];
			$sd_params['social_discount_vk_share_enabled'] = isset($product['social_discount_vk_share_enabled']) ? $product['social_discount_vk_share_enabled'] == 'on' : $sd_params['social_discount_vk_share_enabled'];
			$sd_params['social_discount_fb_like_enabled']  = isset($product['social_discount_fb_like_enabled'])  ? $product['social_discount_fb_like_enabled'] == 'on'  : $sd_params['social_discount_fb_like_enabled'];
			$sd_params['social_discount_gp_like_enabled']  = isset($product['social_discount_gp_like_enabled'])  ? $product['social_discount_gp_like_enabled'] == 'on'  : $sd_params['social_discount_gp_like_enabled'];
			$sd_params['social_discount_mm_like_enabled']  = isset($product['social_discount_mm_like_enabled'])  ? $product['social_discount_mm_like_enabled'] == 'on'  : $sd_params['social_discount_mm_like_enabled'];
			$sd_params['social_discount_ok_like_enabled']  = isset($product['social_discount_ok_like_enabled'])  ? $product['social_discount_ok_like_enabled'] == 'on'  : $sd_params['social_discount_ok_like_enabled'];
			$sd_params['social_discount_tw_like_enabled']  = isset($product['social_discount_tw_like_enabled'])  ? $product['social_discount_tw_like_enabled'] == 'on'  : $sd_params['social_discount_tw_like_enabled'];
			
			$sd_params['social_discount_vk_like_value'] = isset($product['social_discount_vk_like_value']) ? $product['social_discount_vk_like_value'] : $sd_params['social_discount_vk_like_value'];
			$sd_params['social_discount_vk_share_value'] = isset($product['social_discount_vk_share_value']) ? $product['social_discount_vk_share_value'] : $sd_params['social_discount_vk_share_value'];
			$sd_params['social_discount_fb_like_value'] = isset($product['social_discount_fb_like_value']) ? $product['social_discount_fb_like_value'] : $sd_params['social_discount_fb_like_value'];
			$sd_params['social_discount_gp_like_value'] = isset($product['social_discount_gp_like_value']) ? $product['social_discount_gp_like_value'] : $sd_params['social_discount_gp_like_value'];
			$sd_params['social_discount_mm_like_value'] = isset($product['social_discount_mm_like_value']) ? $product['social_discount_mm_like_value'] : $sd_params['social_discount_mm_like_value'];
			$sd_params['social_discount_ok_like_value'] = isset($product['social_discount_ok_like_value']) ? $product['social_discount_ok_like_value'] : $sd_params['social_discount_ok_like_value'];
			$sd_params['social_discount_tw_like_value'] = isset($product['social_discount_tw_like_value']) ? $product['social_discount_tw_like_value'] : $sd_params['social_discount_tw_like_value'];

			$sd_params['social_discount_custom_enabled'] = 'on';
			
			if ($sd_params['social_discount_custom_enabled'] || $sd_custom['social_discount_custom_enabled']) {
				$this->model_catalog_social_discount->setCustomSocialDiscount($update_id, $sd_params);
			}
	...
	private function addProduct(&$raw_prod) {
		...
		$cats = array();
		do  {
		 $cats[0][$i] = $this->field_names['category' . $i];
		 $i++;
		} while (isset($this->field_names['category' . $i]));
		
		foreach ($cats as $category_field) {
			$categories = array();
		...
		$results = $this->model_catalog_attribute->getAttributes($data);
		foreach ($results as $result) {						
				$attr = $this->field_names['product_attribute' . $result['attribute_id']];		
		...
		//echo $this->field_names['product_attribute1'];
		foreach ($this->field_names['product_attribute'] as $attr) {
		//echo $attr . " " . $raw_prod[$attr] . "<br>";
			if (isset($raw_prod[$attr]) && $raw_prod[$attr] != '') {
				$input_attributes[$attr] = $raw_prod[$attr];
			}
		}
		...
		foreach ($this->desc_data as $field => $default_value) {
			...
			//echo $field . " " . $product['product_description'][$this->request->post['csv_import_language']][$field] . "<br>";
		}
		...
		//echo $raw_prod[$this->field_names['keyword']];
		//Product Tags
		if (isset($this->field_names['product_tag']) && isset($raw_prod[$this->field_names['product_tag']])) {
		//echo $raw_prod[$this->field_names['product_tag']];
			$product['product_description'][$this->request->post['csv_import_language']]['tag'] = $raw_prod[$this->field_names['product_tag']];
		} else {
			$product['product_description'][$this->request->post['csv_import_language']]['tag'] = array();
		}
		//echo $product['product_tag'][$this->request->post['csv_import_language']];
		//Product Specials
		...
		if (isset($this->field_names['product_special']) && !empty($raw_prod[$this->field_names['product_special']])) {
			...
			//echo $product['product_special'][0]['price'];
		}
		$this->field_names['price'] = $preserved_price_field_name;
		
		//echo $product['product_special'];
		//Additional Images
		...
		//echo $product['product_image'][0]['image'];
		//Product Rewards
		...
		//скидка за лайк
		 if (isset($raw_prod['vklike'])) {
			$product['social_discount_vk_like_enabled'] = 'on';
			$product['social_discount_vk_like_value'] = $raw_prod['vklike'];
		}
		if (isset($raw_prod['vkshare'])) {
			$product['social_discount_vk_share_enabled'] = 'on';
			$product['social_discount_vk_share_value'] = $raw_prod['vkshare'];
		}
		
		
		if (isset($raw_prod['fblike'])) {
			$product['social_discount_fb_like_enabled'] = 'on';
			$product['social_discount_fb_like_value'] = $raw_prod['fblike'];
		}
		if (isset($raw_prod['google'])) {
			$product['social_discount_gp_like_enabled'] = 'on';
			$product['social_discount_gp_like_value'] = $raw_prod['google'];
		}
		if (isset($raw_prod['mymir'])) {
			$product['social_discount_mm_like_enabled'] = 'on';
			$product['social_discount_mm_like_value'] = $raw_prod['mymir'];
		}
		if (isset($raw_prod['oklike'])) {
			$product['social_discount_ok_like_enabled'] = 'on';
			$product['social_discount_ok_like_value'] = $raw_prod['oklike'];
		}
		if (isset($raw_prod['twitter'])) {
			$product['social_discount_tw_like_enabled'] = 'on';
			$product['social_discount_tw_like_value'] = $raw_prod['twitter'];
		}		
		if (isset($raw_prod['delivery'])) {
			$product['product_additionals'][0][$this->request->post['csv_import_language']]['name'] = "Доставка";
			$product['product_additionals'][0][$this->request->post['csv_import_language']]['description'] = "<p>" . $raw_prod['delivery'] . "</p>";		
		}
		if (isset($raw_prod['video'])) {
			$product['product_additionals'][1][$this->request->post['csv_import_language']]['name'] = "Видео";
			$product['product_additionals'][1][$this->request->post['csv_import_language']]['description'] = "<p>" . $raw_prod['video'] . "</p>";		
		}
...
//echo $key . " " . $value . "<br>";
	...
	private function updateProduct($update_id, &$raw_prod) {
		...
		$product['product_tag'] = $product['product_description'][$this->data['csv_import_language']]['tag'];
		// Product Specials
		...
		$product['status'] = $raw_prod['visibility'];
	...
	private function getNextProduct($file) 
	{
		...
		//echo fgetcsv($this->fh, 0, $this->delim)[0];
		...
		//echo count($this->$row);
//echo array_combine($this->headings, $row)[0];
		...
		//echo $cat;
			if ($cat != '') {
				...
				if ($cat_id == 0) {
					...
					//echo $new_cat['category_description'][2]['name'];
	...
	private function getAttributes($input_attributes) {
		...
		foreach ($input_attributes as $name=>$value) {
			...
			$attr_group_id = $this->model_tool_csv_import->getAttributeGroupId('other');
			if ($attr_group_id == 0) {
			...
			$attr_id = $this->model_tool_csv_import->getAttributeId($name);
			if ($attr_id == 0) {
				//it doesn't exist, let's add it
				$new_attr['attribute_group_id'] = $attr_group_id;
				$new_attr['sort_order'] = 9999;
