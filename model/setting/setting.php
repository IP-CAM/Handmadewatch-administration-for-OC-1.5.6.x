...
class ModelSettingSetting extends Model {
	...
	public function getMytimer() {
		...
		public function setMytimer($data) {
		
		if ($this->db->escape($data['enabled']) == "on") {
			$enabled = 1;
		} else {
			$enabled = 0;
		}
		
		$this->db->query("UPDATE `" . DB_PREFIX . "mytimer` SET `text` = '" . $this->db->escape($data['text']) . "', `enabled` = '" . $enabled . "', `time` = '" . $this->db->escape($data['time']) . "' WHERE 1");
}
}
?>