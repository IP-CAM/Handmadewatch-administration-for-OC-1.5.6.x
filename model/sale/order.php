	...
	public function editOrder($order_id, $data) {
		...
		if ($order_query->num_rows) {
			...
			foreach($product_query->rows as $product) {
				...
				if ($product_data['sku'] == "") {
					$remeshok = $this->model_catalog_product->getRemeshok($product['product_id']);
					$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity + " . (int)$product['quantity'] . ") WHERE product_id = '" . (int)$remeshok['product_id'] . "' AND subtract = '1'");
					$tsiferblat = $this->model_catalog_product->getTsiferblat($product['product_id']);
					$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity + " . (int)$product['quantity'] . ") WHERE product_id = '" . (int)$tsiferblat['product_id'] . "' AND subtract = '1'");			
